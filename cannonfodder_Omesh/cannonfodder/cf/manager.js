	...
	*/
	...
	addTurret: function(xx,yy) {
		Q = gbox.addObject({
			...
			first:function() {
					//angleTurretFace(this, this.fodderList);
				targetFodder(Manager.fodderList, this);
			},
...
function angleTurretFace(turret, fodder){
	...
	//}
	var deltaY = fodder.gy - turret[0].gy;
	var deltaX = fodder.gx - turret[0].gx;
	var angle = (Math.atan(deltaY / deltaX)) * 180 / Math.PI
	var x = Math.floor((angle - 22.5)/45) % 8;
	if(x == 0){
		turret[0].frame = 5;
	}
	else if(x == -1){
...
function targetFodder(fodders, turret){//(fodders, turret){
	...
	targetDistance = Math.sqrt(Math.pow(fodders[0].gx-turret.gx,2)+Math.pow(fodders[0].gy-turret.gy,2))
	//window.alert(targetDistance);
		if(targetDistance <= minRangeToFire){// test if it within the minimum distance from fodder
			if (shotDelay == 10){//can pass a variable to measure the 
			
				//window.alert(turret.gy)
				Manager.addShell(turret.gx, turret.gy);
